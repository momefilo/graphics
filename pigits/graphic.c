...
int main(){
	...
	FILE *fileH = fopen("pigit15x20.h", "w");
	if(fileH==0){printf("Dateifehler h\n");return 0;}
	fprintf(fileH, "#define PIGIT_LEN (15*20*2+1)\n");
	fprintf(fileH, "static uint8_t PIGIT[10][PIGIT_LEN] ={\n");

	//5r6g5b pixelformat (3Ah =03h)
	for(int i=0; i<10; i++){
		char filename[14];
		sprintf(filename, "p%d_15x20.data", i);
